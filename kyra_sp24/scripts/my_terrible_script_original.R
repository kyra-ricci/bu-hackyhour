...
brown <- "#8D6B5B88"
sage <- "#76a87488"
setwd("C:/Users/egrames/Documents/Research/Secondary projects/moth-phenology/iNat Exports ADULTS-20240216T200407Z-001/")
# Read in data -----------------------------------------------------------------
# Read in iNaturalist data
files <- list.files("iNat Exports ADULTS/", pattern=".csv", full.names = T)
filenos <- (1:84)[-c(2, 53)]
...
out <- lapply(files[-c(2,53)], function(x){
  read.csv(unzip(x))
})

issue <- read.csv("./observations-402663.csv")
issue2 <- read.csv("./observations-403042.csv/observations-403042.csv")
issue3 <- read.csv("./observations-402954.csv")

issue4 <- read.csv("./observations-403075.csv")
issue5 <- read.csv("./observations-403076.csv")
issue6 <- read.csv("./observations-403044.csv/observations-403044.csv")


inat <- do.call(rbind, out)
inat <- rbind(inat, issue, issue2, issue3, issue4, issue5, issue6)
...
sp.inat[is.na(sp.inat)] <- sp[is.na(sp.inat)]
...
firstflight <- peakflight <- lastflight <- array(dim=c(nrow(species), 3))
for(i in 1:nrow(species)){
  tmp <- inat[inat$scientific_name==species[i,2],]
  
  firstflight[i,1] <- historical$`1919First`[historical$`Species name`==species[i,1]]
  firstflight[i,2] <- historical$`1922First`[historical$`Species name`==species[i,1]]
  firstflight[i,3] <- min(as.numeric(tmp$week), na.rm=T)
  
  lastflight[i,1] <- historical$`1919Last`[historical$`Species name`==species[i,1]]
  lastflight[i,2] <- historical$`1922Last`[historical$`Species name`==species[i,1]]
  lastflight[i,3] <- max(as.numeric(tmp$week), na.rm = T)
  
}


ff1 <- table(firstflight[,1])
ff2 <- table(firstflight[,2])
ff3 <- table(firstflight[,3])

plot(names(ff1) ~ rep(1, length(ff1)), cex=log(ff1+1), pch=16, xlim=c(0.75, 3.25),
     ylab="First flight week", xlab="Year", axes=F, ylim=c(0, 40), col="white")
...
for(i in 1:nrow(ff12)){
  for(j in 1:ncol(ff12)){
    if(ff12[i,j]>0){
      arrows(x0=1, x1=2, y0=as.numeric(rownames(ff12)[i]), y1=as.numeric(colnames(ff12)[j]),
             lwd=ff12[i,j], col= lav,
             length=0)
    }
  }
}
